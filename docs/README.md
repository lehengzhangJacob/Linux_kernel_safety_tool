# Linux 内核静态分析与可视化平台 (Web SaaS 版)

## 1. 项目简介

本项目旨在构建一个针对 Linux 内核的高精度静态分析与可视化系统。针对操作系统内核代码量大、并发逻辑复杂的特点，我们开发了基于 GCC 插件的底层分析引擎，并在此基础上构建了**现代化的 Web 可视化控制台**，为内核开发者提供“开箱即用”的代码安全审计体验。

### 核心特性
1. **SaaS 风格 Web 控制台**：提供直观的图形化界面，支持本地源码一键上传、实时编译日志追踪、分析进度可视化。
2. **语义级静态分析 (GCC Plugin)**：深入编译器中间表示（GIMPLE）层，精准识别变量作用域，提取函数调用关系。
3. **并发安全检测**：基于静态锁集分析（Lockset Analysis），实现过程间的锁状态追踪，自动检测未受保护的全局变量访问（竞态条件）。
4. **图谱化审计辅助 (Neo4j)**：将数百万行的内核代码转化为可交互的知识图谱，支持复杂路径检索和热点变量分析。
5. **OOM 保护机制**：针对虚拟机环境优化，自动限制编译并发数，确保大规模内核分析的稳定性。

> **通用性说明**：虽然本项目以 Linux v6.6.1 为主要测试对象，但核心分析引擎（GCC 插件）基于编译器中间表示（GIMPLE）构建，不依赖特定内核版本的 API。**经实际测试验证**，该工具链具备**广泛的适用性**，可无缝迁移并成功分析其他版本的 Linux 内核（如 v5.15, v6.1 等），同样适用于其他基于 Kbuild 构建系统的 C/C++ 项目。
...
系统采用**前后端分离 + 松耦合流水线**架构，分为四大核心模块：

```mermaid
graph TD
    subgraph "前端交互层 (Vue.js)"
        UI[Web 控制台]
    end

    subgraph "后端调度层 (Flask)"
        API[API 服务]
        OOM[并发控制/OOM保护]
    end

    subgraph "核心分析层 (GCC Plugin)"
    ...
    subgraph "数据存储与可视化 (Neo4j)"
        E[Python ETL 脚本] -->|转换| C[CSV 文件]
        C -->|导入| N[Neo4j 图数据库]
    end
    ...
    style UI fill:#6bf,stroke:#333,stroke-width:2px
```
...
### 3. 安装 Python 依赖 (Web 后端)

Web 控制台后端基于 Flask 构建，需要安装相关 Python 依赖：
...
Web 控制台前端基于 Vue.js 构建，需要安装 Node.js 依赖：
...
本项目提供了一个现代化的 SaaS 风格 Web 控制台，支持本地源码上传、实时编译日志查看以及可视化报告跳转。这是**最推荐**的使用方式。

**前置条件：**
- 已完成 Python 后端依赖安装（见上文"安装 Python 依赖"）
- 已完成前端依赖安装（见上文"安装前端依赖"）
...
**Web 控制台核心工作流：**
1. **上传源码**：在网页端选择本地的 Linux 内核源码文件夹（如 `linux-6.6.1`），系统会分批上传至服务器。
2. **启动分析**：点击"开始分析"，后端会自动调用 GCC 插件进行编译和静态分析。
3. **实时监控**：在网页端实时滚动显示编译日志和分析进度。
4. **查看报告**：分析完成后，点击"查看可视化报告"跳转至 Neo4j 图数据库界面（默认端口 7474）。
...
如果您需要在 CI/CD 环境中运行，或者更喜欢命令行操作，可以使用 `full_run.sh` 脚本。该脚本可以自动完成清理、编译、分析、数据导入和数据库启动的所有步骤：
...
| 脚本名称              | 功能描述                                                                                | 典型使用场景                                     |
| :-------------------- | :-------------------------------------------------------------------------------------- | :----------------------------------------------- |
| `web_dashboard/app.py`| **Web 控制台后端**。启动 Flask 服务，提供源码上传、分析调度和实时日志查看功能。   | 日常使用，通过浏览器进行可视化交互分析。         |
| `full_run.sh`       | **一键启动脚本**。串行执行清理、编译分析、数据转换、数据库导入和启动服务。        | 命令行模式下首次运行或需要全量重新分析时使用。   |
